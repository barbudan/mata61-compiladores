...
int main() {
	...
	*/
	...
	pilha.push(make_pair(0,"")); // estado 0 adicionado na pilha
	...
	*/
	...
	list <pair<int,string>> teste; // lista teste para imprimir pilha
	...
	while(!pilha.empty())
	{
		...
		*/if(tabela.find(make_pair(topo.first, (*it).id) ) == tabela.end())
		...
		if(is_shift(s) && (*it).id!="$")
		{
			pilha.push(make_pair(to_int(s), (*it).id));
			it++;
		}
		else if(is_reduce(s) || (*it).id=="$")
		{
			...
			pilha.push(make_pair(new_estado_pilha, cabeca_regra));
		}
