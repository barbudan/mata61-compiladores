...
Node *cria_raiz(Token tk);
Node *new_node(Token tk, Node *pai);
Node *new_node_sem_pai(Token tk);
void imprime_arvore(Node *raiz);
void imprime_arvore_dfs(Node *raiz);
void preenche_cabeca_regra(set <string> &regras);
...
void percorre_arvore_dfs(Node *raiz);
...
void imprime_arvore_dfs(Node *raiz){
	...
	for(int i=0;i<raiz->filhos.size();i++)
		imprime_arvore_dfs(raiz->filhos[i]);
...
void percorre_arvore_dfs(Node *raiz){
	...
	for(int i=0;i<raiz->filhos.size();i++)
		percorre_arvore_dfs(raiz->filhos[i]);
...
void testa_atribuicao(Node *raiz){
	...
	if(raiz->token.data=="DECLARACAO" && flag==1)
	{
		flag=0;
		return;
	}
